...
Model::update()
{
    ...
    #pragma omp parallel for // Parallélisation de la boucle avec OpenMP
    for (std::size_t i = 0; i < keys.size(); ++i)
    {
        auto f = m_fire_front[keys[i]];
        // Récupération de la coordonnée lexicographique de la case en feu :
        LexicoIndices coord = get_lexicographic_from_index(keys[i]);
        // Et de la puissance du foyer
        double power = log_factor(f);

        // On va tester les cases voisines pour contamination par le feu :
        if (coord.row < m_geometry-1)
        {
            double tirage = pseudo_random(keys[i] + m_time_step, m_time_step);
            double green_power = m_vegetation_map[keys[i] + m_geometry];
            double correction = power * log_factor(green_power);
            if (tirage < alphaSouthNorth * p1 * correction)
            {
                #pragma omp critical
                {
                    m_fire_map[keys[i] + m_geometry] = 255.;
                    next_front[keys[i] + m_geometry] = 255.;
                }
            }
        }
        ...
        if (coord.row > 0)
        {
            double tirage = pseudo_random(keys[i] * 13427 + m_time_step, m_time_step);
            double green_power = m_vegetation_map[keys[i] - m_geometry];
            double correction = power * log_factor(green_power);
        ...
        {
            double tirage = pseudo_random(keys[i] * 13427 * 13427 + m_time_step, m_time_step);
            double green_power = m_vegetation_map[keys[i] + 1];
            double correction = power * log_factor(green_power);
            if (tirage < alphaEastWest * p1 * correction)
            {
                #pragma omp critical
                {
                    m_fire_map[keys[i] + 1] = 255.;
                    next_front[keys[i] + 1] = 255.;
                }
            }
        }
        ...
        if (coord.column > 0)
        {
            double tirage = pseudo_random(keys[i] * 13427 * 13427 * 13427 + m_time_step, m_time_step);
            double green_power = m_vegetation_map[keys[i] - 1];
            double correction = power * log_factor(green_power);
        ...
        {   // On regarde si il commence à faiblir pour s'éteindre au bout d'un moment :
            double tirage = pseudo_random(keys[i] * 52513 + m_time_step, m_time_step);
            if (tirage < p2)
            {
                #pragma omp critical
                {
        ...
        else
        {
            ...
            #pragma omp critical
            {
                m_fire_map[keys[i]] >>= 1;
                next_front[keys[i]] >>= 1;
                if (next_front[keys[i]] == 0)
                {
                    next_front.erase(keys[i]);
                }
            }
        }
    }
    ...
    for (std::size_t i = 0; i < keys.size(); ++i)
    {
        if (m_vegetation_map[keys[i]] > 0)
            m_vegetation_map[keys[i]] -= 1;
    }
    m_time_step += 1;
