...
int main( int nargs, char* args[] )
{
    ...
    // Variables pour accumuler les durées
    unsigned long simulationSteps = 0;
    std::chrono::duration<double, std::micro> totalSimUpdateTime(0);
    std::chrono::duration<double, std::micro> totalDisplayTime(0);
    
    // Mesure du temps global de la simulation
    auto overallStart = std::chrono::high_resolution_clock::now();
    
    bool simulationContinues = true;
    while (simulationContinues)
    {
        // Mesurer le temps de mise à jour du modèle (pas de temps)
        auto startSim = std::chrono::high_resolution_clock::now();
        simulationContinues = simu.update();
        auto endSim = std::chrono::high_resolution_clock::now();
        totalSimUpdateTime += (endSim - startSim);
        ...
        totalDisplayTime += (endDisplay - startDisplay);
        
        // Affichage périodique d'information sur le temps de simulation
        if ((simu.time_step() & 31) == 0) 
            std::cout << "Time step " << simu.time_step() << "\n===============" << std::endl;
        
        // Gestion des événements SDL (permet de quitter)
    ...
    std::cout << "\n=== Bilan de la simulation ===" << std::endl;
    std::cout << "Nombre total de pas de temps : " << simulationSteps << std::endl;
    std::cout << "Temps total de la simulation : " << overallDuration.count() << " millisecondes" << std::endl;
    std::cout << "Temps moyen pour la mise a jour du modele (pas de temps) : " 
              << (totalSimUpdateTime.count() / simulationSteps) << " microsecondes" << std::endl;
    std::cout << "Temps moyen pour l'affichage : " 
              << (totalDisplayTime.count() / simulationSteps) << " microsecondes" << std::endl;
